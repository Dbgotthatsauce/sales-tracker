...
import {
  ...
  FolderSync, CalendarPlus, Trophy, Euro, Search,
} from 'lucide-react'
...
function getStartDate(filter: Filter): Date | null {
  ...
  if (filter === 'week') {
    // ISO-Woche: Montag = 1, Sonntag = 0 → Montag berechnen
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-800 p-4 flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="text-2xl font-bold tabular-nums shrink-0 text-slate-100">
          {rate}%
        </span>
      </div>
...
export default function DashboardPage() {
  ...
  // Nutzer aus user_profiles-View laden (einmalig beim Mount)
  useEffect(() => {
  ...
  async function loadData(activeFilter: Filter = filter, activeUserId: string = selectedUserId) {
    setLoading(true)
    setError(null)

    // Hinweis: RLS ist aktiv. Für das öffentliche Dashboard entweder:
    // a) Eine SELECT-Policy ohne auth.uid()-Prüfung hinzufügen (z.B. FOR SELECT USING (true)), oder
    // b) Den Supabase-Client mit dem service_role-Key initialisieren (nur serverseitig verwenden!)
    let query = supabase
  ...
  // Neu laden wenn sich Zeit- oder Nutzerfilter ändert
  useEffect(() => { loadData(filter, selectedUserId) }, [filter, selectedUserId])

  const get = (key: string) => totals[key] ?? 0

  // Gibt immer einen Prozentwert zurück; 0.0 bei Division durch 0
  ...
  // Für zusammengesetzte Zähler / Nenner
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <header className="border-b border-slate-800 bg-slate-950 sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-6 py-4 flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Aggregierte KPIs aller Aktivitäten</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              {lastUpdated && (
          ...
          {/* Filterleiste: Zeitfilter + Nutzer-Dropdown */}
          <div className="flex items-center gap-3 flex-wrap">

            {/* Zeitfilter Toggle-Group */}
        ...
        {error && (
          <div className="rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-sm text-slate-300">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Tipp: Für das Dashboard eine RLS-Policy <code className="bg-slate-800 px-1 rounded">FOR SELECT USING (true)</code> hinzufügen.
            </p>
          </div>
        )}
        ...
        {!loading && activeTab === 'cold-calling' && (
          <>
            <section>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                <KpiCard label="Anwahlen"               value={fmtNum(get('Anwahlen'))}                 icon={<Phone size={16} />} />
                <KpiCard label="Erreichte Personen"     value={fmtNum(get('Erreichte Personen'))}       icon={<UserCheck size={16} />} />
                <KpiCard label="Entscheider"            value={fmtNum(get('Entscheider'))}              icon={<Briefcase size={16} />} />
                <KpiCard label="An Vorzimmer gescheit." value={fmtNum(get('An Vorzimmer gescheitert'))} icon={<DoorClosed size={16} />} />
                <KpiCard label="Intro"                  value={fmtNum(get('Intro'))}                    icon={<Handshake size={16} />} />
                <KpiCard label="Short Story"            value={fmtNum(get('Short Story'))}              icon={<BookOpen size={16} />} />
                <KpiCard label="Pitch"                  value={fmtNum(get('Pitch'))}                    icon={<Target size={16} />} />
                <KpiCard label="Nach Termin gefragt"    value={fmtNum(get('Nach Termin gefragt'))}      icon={<CalendarCheck size={16} />} />
                <KpiCard label="Termin vereinbart"      value={fmtNum(get('Termin vereinbart'))}        icon={<CheckCheck size={16} />} />
                <KpiCard label="Nachqualifizierung"     value={fmtNum(get('Nachqualifizierung'))}       icon={<Search size={16} />} />
              </div>
            </section>
        ...
        {!loading && activeTab === 'setting' && (
          <>
            <section>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                <KpiCard label="Setting geführt"        value={fmtNum(get('Setting geführt'))}        icon={<MessageSquare size={16} />} />
                <KpiCard label="Setting Unqualifiziert" value={fmtNum(get('Setting Unqualifiziert'))} icon={<XCircle size={16} />} />
                <KpiCard label="No Show"                value={fmtNum(get('No Show'))}                icon={<Ghost size={16} />} />
                <KpiCard label="Setting Follow Up"      value={fmtNum(get('Setting Follow Up'))}      icon={<RefreshCw size={16} />} />
                <KpiCard label="Closing terminiert"     value={fmtNum(get('Closing terminiert'))}     icon={<PhoneForwarded size={16} />} />
              </div>
            </section>
          ...
          <>
            ...
            <div className="rounded-xl border border-slate-600 bg-slate-800 p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-slate-100 tabular-nums">{fmtEur(get('Betrag'))}</p>
              </div>
            </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                <KpiCard label="Closing geführt"           value={fmtNum(get('Closing geführt'))}           icon={<MessageSquare size={16} />} />
                <KpiCard label="Closing No Show"           value={fmtNum(get('Closing No Show'))}           icon={<Ghost size={16} />} />
                <KpiCard label="Closing Follow Up"         value={fmtNum(get('Closing Follow Up'))}         icon={<FolderSync size={16} />} />
                <KpiCard label="Folgebesprechung ver."     value={fmtNum(get('Folgebesprechung vereinbart'))} icon={<CalendarPlus size={16} />} />
                <KpiCard label="Folgebesprechung No Show"  value={fmtNum(get('Folgebesprechung No Show'))}  icon={<CalendarX size={16} />} />
                <KpiCard label="Als Kunden gewonnen"       value={fmtNum(get('Als Kunden gewonnen'))}       icon={<Trophy size={16} />} />
              </div>
            </section>
